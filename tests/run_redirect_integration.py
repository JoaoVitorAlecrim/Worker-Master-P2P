...
proc_b = subprocess.Popen(['python', master_path], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env_b, text=True)
...
proc_a = subprocess.Popen(['python', master_path], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env_a, text=True)
...
proc_w = subprocess.Popen(['python', worker_path, worker_uuid], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env_w, text=True)
...
try:
    ...
    while time.time() - start < 20:
        for name, p in procs.items():
            ...
            if name == 'W' and 'Redirecionamento recebido' in line or '↪ Redirecionamento' in line or '↪ Redirecionamento recebido' in line or '↪ Redirecionamento' in line:
                redirected = True
