...
class TaskManager:
    ...
    def save_state(self, server_uuid: str) -> None:
        ...
        with self.lock:
            data = {
                ...
                "logs": [log.to_dict() for log in self.task_logs],
            }
            ...
            with self.lock:
                ...
                for ld in data.get("logs") or []:
                    log = TaskLog(
                        ...
                        timestamp=ld.get("timestamp", 0),
                        event_type=ld.get("event_type", ""),
                        worker_uuid=ld.get("worker_uuid"),
    ...
    def create_task(self, operation: str, values: List[Any]) -> Task:
        ...
        with self.lock:
            ...
            self._log_event(task.task_id, "created", details={"operation": operation, "values": values})
            try:
    ...
    def assign_task(self, task_id: str, worker_uuid: str) -> bool:
        ...
        with self.lock:
            ...
            self._log_event(
                task_id, "assigned", worker_uuid=worker_uuid, details={"worker": worker_uuid, "retry": task.retries}
            )
            try:
                if self.persistence_server_uuid:
                    self.save_state(self.persistence_server_uuid)
    ...
    def complete_task(self, task_id: str, result: Any) -> bool:
        ...
        with self.lock:
            ...
            self._log_event(
                task_id, "completed", worker_uuid=task.assigned_worker, details={"result": str(result)[:100]}
            )
            try:
                if self.persistence_server_uuid:
                    self.save_state(self.persistence_server_uuid)
    ...
    def fail_task(self, task_id: str, error_message: str) -> bool:
        ...
        with self.lock:
            ...
            self._log_event(task_id, "failed", worker_uuid=task.assigned_worker, details={"error": error_message})
            try:
                if self.persistence_server_uuid:
                    self.save_state(self.persistence_server_uuid)
            except Exception:
    ...
    def reassign_task(self, task_id: str) -> bool:
        ...
        with self.lock:
            ...
            self._log_event(
                task_id, "reassigned", worker_uuid=old_worker, details={"old_worker": old_worker, "retry": task.retries}
            )
            try:
                if self.persistence_server_uuid:
                    self.save_state(self.persistence_server_uuid)
    ...
    def get_tasks_by_worker(self, worker_uuid: str) -> List[Task]:
        ...
        with self.lock:
            return [task for task in self.tasks.values() if task.assigned_worker == worker_uuid]
    ...
    def register_worker(
        self, worker_uuid: str, server_uuid: str, host: Optional[str] = None, free_disk_bytes: Optional[int] = None
    ) -> Worker:
    ...
    def get_statistics(self) -> dict:
        ...
        with self.lock:
            ...
            return {
                ...
                "workers": {"online": online_workers, "offline": offline_workers, "total": len(all_workers)},
    ...
    def _log_event(
        self, task_id: str, event_type: str, worker_uuid: Optional[str] = None, details: Optional[dict] = None
    ) -> None:
        """Registra evento em log."""
        log = TaskLog(task_id=task_id, event_type=event_type, worker_uuid=worker_uuid, details=details or {})
        self.task_logs.append(log)
